class Mobil {
    ...
    gas() {
        if (
            this.kecepatanSaatIni < this.kecepatanMax &&
            this.isIncrement == true
        ) {
...
function tambahMobil() {
    ...
    const kecepatanMax = parseInt(
        prompt("Masukkan kecepatan maksimal mobil:")
    );

    if (jenisMobil == 1) {
        const kecepatanTurbo = parseInt(
            prompt("Masukkan kecepatan turbo mobil:")
        );
...
function tombolDiklik(nomorTombol) {
    var output = document.getElementById("output");
    if (nomorTombol === 1) {
        output.innerHTML = "Mobil Toyota";
    } else if (nomorTombol === 2) {
        output.innerHTML = "Mobil Ferrari";
    }
}